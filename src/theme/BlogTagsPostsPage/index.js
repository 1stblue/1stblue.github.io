    ...
    <BlogLayout sidebar={sidebar}>
      ...
      >
        ...
        <div className={clsx('pt-0 blog-md:pt-16')}>
          <div className="text-gray-500 ">Posts tagged with</div>
          <h1 className="!mb-0">{tag.label}</h1>
        </div>
        <BlogPostItems items={items} showTitle={false} isTagsPage={true} />
        <div className="blog-md:border-t border-t-gray-200 ">
          <BlogListPaginator metadata={listMetadata} basePath={`/blog/tags/${tag.label}`} />
        </div>
      </div>
    </BlogLayout>
  );
