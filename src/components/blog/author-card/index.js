...
const AuthorCard = ({ author }) => {
  const authorHasSocialInfo = author.github || author.twitter || author.linkedin;
  ...
  return (
    ...
    >
      <div className={clsx('flex', 'justify-between', 'items-center')}>
        <div className={clsx('w-full', 'flex items-center justify-between flex-wrap')}>
          <div className={clsx('flex items-center', 'gap-2 sm:gap-6')}>
            <Link to={`/blog/author/${author?.key}`} itemProp="url" className="flex-shrink-0">
              <img
                src={author?.imageURL}
                alt={author?.name}
                loading="lazy"
            ...
            <div className={clsx('not-prose flex flex-col justify-between')}>
              <h1
                className={clsx(
                  ...
                  'm-0 p-0 pb-2 font-bold text-gray-900'
                )}
              >
                {author?.name}
              </h1>
              <div className={clsx('text-xs sm:text-base', 'text-gray-600')}>{author?.title}</div>
            </div>
          </div>
