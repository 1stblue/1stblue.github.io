  ...
  return (
    ...
    >
      <div className="blog-sm:px-6 w-full px-4">
        <h2 className="m-0 mb-4 p-0 pl-4 text-2xl font-semibold text-gray-900 ">{title}</h2>
        <div className="flex flex-col">
          {posts.map(post => (
            <div
              key={post.permalink ?? post.id}
              className={clsx('flex', 'flex-col', 'p-5', 'mb-5', 'rounded-md', 'bg-gray-50 ')}
            >
              <Link
                to={post.permalink}
                rel="dofollow"
              ...
              <p className={clsx('font-sm', 'text-gray-700 ')}>{post.description}</p>

              <div id="post-info" className="flex items-center gap-2 text-sm text-gray-600 ">
