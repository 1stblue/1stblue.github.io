...
import { isActiveSidebarItem, isSamePath, useDocsSidebar } from '@docusaurus/theme-common/internal';
...
const SidebarCategory = ({
  ...
  const [collapsed, setCollapsed] = React.useState(collapsible === false ? false : !(isHeader || isActive));
  ...
  return (
    <div
      className={clsx(
        ...
        line && 'pl-5',
        line && 'ml-[11px]',
        line && 'border-l-2 border-l-gray-200 '
      )}
    >
      <Comp
        type="button"
        ...
        className={clsx(
          ...
          'appearance-none',
          'focus:outline-none',
          isActive && !isHeader ? ' text-gray-900' : 'text-gray-500',
          isHeader ? 'uppercase' : '',
          'font-normal',
          'flex items-center',
          'py-1',
          'pr-0.5',
          isHeader ? 'text-xs leading-4 tracking-widest' : 'text-sm leading-6',
          'relative',
          !isHeader && 'group',
          'transition-colors duration-200 ease-in-out',
          !isHeader && ' hover:text-gray-900',
      ...
      >
        <div
          className={clsx(
            ...
            {
              'bg-gray-100/50 ': isActive && isSame,
  ...
  return (
    <Link
      ...
      className={clsx(
        'relative',
        'min-h-[28px]',
        isShiny && 'bg-sidebar-item-shiny-light  rounded-xl',
        isShiny ? 'text-bluepipe-blue ' : isActive ? ' text-gray-900' : 'text-gray-500 ',
        !isShiny && ' hover:text-gray-900',
...
const SidebarHtml = ({
  ...
  return (
    ...
    >
      <span
        className={clsx('z-[1]', '-ml-1', 'px-1', 'bg-gray-0')}
  ...
  return (
    items?.map((item, index) => {
      switch (item.type) {
        ...
        case 'html':
          return <SidebarHtml key={index} item={item} path={path} line={!!line} variant={variant} />;
        case 'link':
          return (
            <SidebarLink
              key={index}
...
export const useSidebarItems = ({ variant, onLinkClick, deferred }: UseSidebarItemsProps) => {
  const sidebar = useDocsSidebar() as SidebarType;
  const { pathname } = useLocation();
