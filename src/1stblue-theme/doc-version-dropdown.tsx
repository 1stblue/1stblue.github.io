...
import { TriangleDownIcon } from '@site/src/1stblue-theme/icons/triangle-down';
        ...
        {({ open }) => (
          <>
            <span className="rounded-md shadow-sm">
              ...
              >
                <span className={clsx('text-gray-500 ', 'text-base', 'block')}>
                  {docContext.activeVersion.label}
                </span>
                <TriangleDownIcon className="text-gray-400 " />
              </Menu.Button>
            </span>
            ...
            >
              <Triangle
                className={clsx('absolute right-[2px] top-[-6px] origin-bottom-right', 'text-gray-700 ')}
              />
              <Menu.Items
                static
                className={clsx(
                    ...
                    <Menu.Item key={version.label}>
                      {() => {
                        ...
                        return (
                          <Link
                            to={version.to}
                            className={clsx('px-2 py-1', 'rounded-[4px]', 'no-underline', {
                              'text-white hover:text-white': isActive,
                              'bg-bluepipe-blue': isActive,
                              'text-gray-300 ': !isActive,
                              'hover:bg-gray-600  hover:text-gray-300 ': !isActive
                            })}
                          >
                            {version.label}
                          </Link>
